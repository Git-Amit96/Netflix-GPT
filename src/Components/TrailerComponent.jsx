...
const TrailerComponent = () => {
  ...
  const { title, overview } = useSelector(store => store.movies.popularMovies?.results?.[1] || {});
  useMovieTrailer();
  return trailerData ? (
    <div className='bg-gradient-to-b from-black via-black/[0.55] to-black'>
      <div className='absolute top-0 left-0 w-[100%]  h-[105vh] box-border z-5  flex  items-center overflow-y-hidden opacity-60'>
      ...
      <div className=' absolute w-[30%] text-white mt-[30vh] opacity-100 pl-[6rem]'>
        <h1 className='text-5xl font-bold mb-[0.3em]'>{title}</h1>
        <p className='text-md font-normal '>{overview}</p>
        <div className='mt-[1.3em] flex justify-between w-[70%]'>
